...
public interface OrderRepositoryInterface extends JpaRepository<Order, Long>{
    ...
    public Order getByCourierAndStatus(Courier courier, String status);
    ...
    public List<Order> getByStatusOrderByTimeAsc(String status);

    @Query("SELECT order.courier FROM Order order WHERE order.status =?1")
    public List<Courier> getAllCouriersByStatus(String status);
